...
export default function RangeFilter({
  ...
  return (
    <div className="soc-range-filter flex min-w-0 shrink-0 items-center">
      <div
        className={`hidden min-[640px]:flex h-7 items-center gap-0.5 rounded-lg border border-[var(--soc-border)] bg-[var(--soc-card)] p-0.5 min-[900px]:h-8 ${
          dimmed ? "opacity-50" : ""
        } ${className}`}
      >
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`rounded-md border border-transparent px-1.5 py-1 text-[9px] font-medium leading-none whitespace-nowrap transition-colors min-[700px]:px-2 min-[700px]:text-[10px] min-[900px]:px-2.5 min-[900px]:py-1.5 min-[900px]:text-[11px] ${
                active
